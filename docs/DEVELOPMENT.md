...
src/
  ...
  keymap.rs               Action SSOT: config name, description, default keys
                          (the chords come from ratada::keymap)
...
The reusable ratatui toolkit and the framework-agnostic theming layer live in the external `ratada` crate, consumed as a path dependency (`ratada = { path = "../../libs/ratada", version = "0.4" }`). hop uses it for: the terminal guard and event model (`Tui`, `TuiEvent`, re-exported by `tui/terminal.rs`), theming (`ratada::theme::{Color, Palette, Skin, Glyphs, GlyphVariant, ThemeColors, ThemeRegistry}`, re-exported as `crate::theme` in `src/lib.rs`), the panel-frame building blocks (`tabs`, `shortcut_hints`, `overlay::dim`, `style`, `chrome::{modal_block, border_title}`), the chord layer under hop's action catalog (`keymap::{Action, Keymap, KeyChord, Conflict}` – see [Keys, theming and glyphs](#keys-theming-and-glyphs)), text editing and its block caret (`input::InputField`, plus `input::is_command` – the one honest test for "is this a command chord?", which a bare `CONTROL` check is not, because `AltGr` reports as `Control`+`Alt` and must keep typing), and the small shared primitives (`nav::cycle`, `text::truncate`, `list`, `scroll`, `gauge`, `spinner`, `clipboard`, `quit`). It is the single source of truth for the look; build new screens on it rather than reimplementing navigation, scrollbars or theming. `nucleo-matcher` stays a **direct** dependency (not routed through `ratada`) because the pure `domain::filter` must not depend on a UI toolkit; `tui/presentation.rs::highlight_name` uses it directly too, so the table's match highlighting and the filter agree on what matched.
...
`keymap.rs` is a top-level layer (not under `tui`) so it can be the single source of truth for shortcuts: the `ACTIONS` table carries each shortcut's config name, description and default keys. The **chord layer itself is the toolkit's**: `keymap.rs` implements `ratada::keymap::Action` for its `Action` enum and hands the catalog over, and `Keymap` is a type alias for `ratada::keymap::Keymap<Action>` (`KeyChord` and `Conflict` are re-exports). The chord grammar, the defaults-vs-overrides merge and the display form therefore live in `ratada`, once, rather than in every consuming app – `from_overrides`, `action_for`, `keys_for`, `hints` and `conflicts` all come from there. The map is resolved once into `App::keymap`. Adding a shortcut means adding a row to `ACTIONS`, nothing else.

It drives **both dispatch and the hints**. `App::handle_list_key` resolves the key with `Keymap::action_for` and then matches on the `Action`, so a `[keys]` override really rebinds. Context-dependent actions branch on the active tab in `App::run_action`, not on the key: `Sort` jumps to a section where the list is sectioned, `Reload`/`ReloadFetch` re-check paths on the Files tab, `ManageSections` only applies there. Only **two** keys stay outside the keymap, consumed by `App::handle_untracked_key` first: `Tab`/`BackTab` and `Esc`, which are structural rather than user-facing actions and are deliberately not rebindable. `Ctrl+Q` never reaches dispatch at all: the `Tui` turns it into `TuiEvent::Quit`. `Shift`+arrow used to be a third such key, because hop's own `KeyChord` ignored the shift modifier and would have resolved a shifted arrow to a plain cursor move; `ratada::keymap` compares shift for a non-character key (a character carries its shift in its case, `G` rather than `shift+g`), so extending the selection is now the ordinary rebindable `extend_up`/`extend_down` binding.

The **footer hints share the same source**: `tui/bindings.rs` lists, per tab, which `Action`s appear in each labelled group (`Open`, `Manage`, `Backup`, `View`, `Git`/`Sections`+`Paths`), and `App::hint_groups` turns each group into `(key, description)` pairs via `keymap.hints(actions)`. A compact static `Navigation` group is prepended (so arrows render as `↑↓ move` rather than the verbose per-key form).
...
**Decision.** Keep the non-blocking overlay state machine; do not swap the overlays for `ratada`'s blocking widgets. Instead, restyle hop's own overlay widgets to look clibase-native: every overlay draws over the **dimmed live view** (`appframe::dim_backdrop`), and each box uses `ratada::chrome::modal_block` so the title reads as an inset border label (`╭─ Title ─`, bold accent). The same applies to the add/edit form, which keeps its own `RepoForm` (path-picker hookup, kind/section cycles, inline slug validation), restyled rather than rebuilt on `ratada::form::Form`. So the **path picker is hop's own `PathPicker`** (`tui/path_picker.rs`), not `ratada::path_picker` – but it mirrors the toolkit picker's look by reusing `ratada`'s public pieces (`list::render` for the highlighted, scrollable list; `chrome::{position_badge, render_badge}` for the `xx/yy` badge in the bottom-right border) and matches its behaviour (`Ctrl+h` toggles hidden dot-entries, hidden by default).

**The help overlay.** `tui/help.rs` stays a non-blocking overlay for the same reason, but is styled to be indistinguishable from `ratada::help`'s: uppercase section headers in `accent_dim` + bold, an aligned `accent` + bold key column, `style::secondary` descriptions and a `shortcut_hints::lines` footer. Its sections mirror the footer's hint groups. A section whose keys a `[keys]` override can move is **not** spelled out in `help::SECTIONS`: the caller hands both in as a `help::HelpSections`, built from the same tokens the footer uses – the trailing `Global` one by `tui::mod::global_group` (`keymap.hints(&[Help, Quit])` + `shortcut_hints::global_bindings()`), the leading `Navigation` one by `navigation_group`, whose selection rows resolve through `keymap.keys_for`. So `?`, `q`, `F1`, `Ctrl+Q`, `Space` and `Shift`+arrow cannot drift from what the keys actually do. `navigation_group` uses `keys_for` rather than `keymap.hints` because the overlay keeps its own fuller wording ("extend the selection range") where the footer is terse ("extend"); only the key comes from the keymap. Its remaining rows (`1 / 2`, `↑ ↓`, `g / G`, `PgUp/PgDn · Ctrl+u/d`) stay static in `NAVIGATION_ROWS` on purpose: each merges several actions into one legible row, exactly as the footer's compact Navigation group does. What hop gives up against `ratada::help` is the fuzzy filter and section navigation.
