...
`keymap.rs` is a top-level layer (not under `tui`) so it can be the single source of truth for shortcuts: `Action` carries each shortcut's config name, description and default keys, and `Keymap::from_overrides(&config.keys)` applies the `[keys]` config overrides and reports conflicts. The map is resolved once into `App::keymap`.

It drives **both dispatch and the hints**. `App::handle_list_key` resolves the key with `Keymap::action_for` and then matches on the `Action`, so a `[keys]` override really rebinds. Context-dependent actions branch on the active tab in `App::run_action`, not on the key: `Sort` jumps to a section where the list is sectioned, `Reload`/`ReloadFetch` re-check paths on the Files tab, `ManageSections` only applies there. Three keys stay outside the keymap and are consumed by `App::handle_untracked_key` first – `Shift`+arrow (extend selection), because `KeyChord` ignores the shift modifier and would otherwise resolve to a plain cursor move, plus `Tab`/`BackTab` and `Esc`, which are structural rather than user-facing actions. `Ctrl+Q` never reaches dispatch at all: the `Tui` turns it into `TuiEvent::Quit`.
...
**The help overlay.** `tui/help.rs` stays a non-blocking overlay for the same reason, but is styled to be indistinguishable from `ratada::help`'s: uppercase section headers in `accent_dim` + bold, an aligned `accent` + bold key column, `style::secondary` descriptions and a `shortcut_hints::lines` footer. Its sections mirror the footer's hint groups, and the trailing `Global` section is built by `tui::mod::global_group` from the same tokens the footer uses (`keymap.hints(&[Help, Quit])` + `shortcut_hints::global_bindings()`), so `?`, `q`, `F1` and `Ctrl+Q` can never drift apart. What hop gives up against `ratada::help` is the fuzzy filter and section navigation.
