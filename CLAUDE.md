...
- **Fetch / startup status**: `tui::StartupStatus` (`Cached` | `Refresh { fetch }`) decides what `App::new` does on start; `cli::startup_status` derives it from `--cached` (cache only, no git), `--fetch` and the `fetch_on_start` config key (`fetch = --fetch || fetch_on_start`). In the TUI `R` = `git fetch` + reload, `r` = reload only (`reload_status`). `hop <slug> --fetch` fetches the repo before launching the tool. `GitClient::fetch` runs `git fetch --quiet` and must capture output / skip missing paths (otherwise git's stderr corrupts the alternate screen).
- **Status bar (two lines)**: the local line (`render_info`) shows entry count, sort and the cache `generated_at`, replaced by a `LineGauge` progress bar while `loading`; the remote line (`render_remote`) shows the last `git fetch` time and warns (amber `colors::CHANGES`) when older than 24h or never. `fetched_at` is persisted in the git-info cache (`cache::save(.., fetched_at)`), updated only when a refresh actually fetched (`App::refresh_fetched`). During a refresh, rows not yet updated this pass show a pending `…` in the status column (`TableView::refreshed`, built from `App::collected`).
