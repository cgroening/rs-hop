...
`hop` is a fuzzy-finder TUI in **Rust (edition 2024)** to jump between git repositories and folders. It is a port of the Python `git-repo-jumper`: pick an entry, its path is written to a handoff file the shell wrapper reads to `cd`, and (for git repos, on TUI `L`/`l`) the configured git tool (lazygit) can be launched. On top of the original it adds in-app management, three tabs, slugs for a `hop <slug>` fast jump, sort modes, and a marker plus picker for paths that no longer exist.
...
src/
  ...
  tui/
    ...
    presentation.rs IconSet (Unicode/ASCII glyphs), status_text, truncate,
                  name/slug + fuzzy-highlight spans, empty hint
