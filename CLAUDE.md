...
`ratatui` + `crossterm` (TUI), `nucleo-matcher` (fuzzy filter), `clap` (CLI/slug), `serde` + `toml` (config + cache + usage), `toml_edit` (comment-preserving config rewrites), `chrono` (timestamps), `thiserror` (domain errors), `anyhow` (binary edge if needed), `log` (facade; file backend in `util/logging.rs`), `unicode-width` (column alignment).
